...
static int test_init_starts_with_empty_resources(void)
{
    ...
    REQUIRE_TRUE(app_tasks_telemetry_queue_count(&context) == 0u, "telemetry queue should start empty");
...
static int test_command_task_reports_parse_error(void)
{
    ...
    REQUIRE_TRUE(app_tasks_telemetry_queue_count(&context) == 1u, "parse errors should queue telemetry");
    REQUIRE_TRUE(!output_contains("ERR_MISSING_ARGUMENT"), "parse error should wait for telemetry task");
    REQUIRE_TRUE(app_telemetry_task_step(&context) == APP_TASK_STATUS_OK, "telemetry task should drain parse error");
    REQUIRE_TRUE(output_contains("ERR_MISSING_ARGUMENT"), "parse error should be reported through telemetry");
    REQUIRE_TRUE(app_tasks_telemetry_queue_count(&context) == 0u, "telemetry queue should drain after send");
    return 0;
}
...
static int test_motion_task_processes_move_and_updates_position(void)
{
    ...
    REQUIRE_TRUE(app_tasks_telemetry_queue_count(&context) == 1u, "MOVE response should queue telemetry");
    REQUIRE_TRUE(!output_contains("OK MOVE QUEUED"), "motion task should not print directly");
    REQUIRE_TRUE(app_telemetry_task_step(&context) == APP_TASK_STATUS_OK, "telemetry task should emit queued response");
    REQUIRE_TRUE(output_contains("OK MOVE QUEUED"), "motion task should report queued move");
    REQUIRE_TRUE(app_tasks_telemetry_queue_count(&context) == 0u, "telemetry queue should drain after response");
...
static int test_estop_and_clear_fault_flow(void)
{
    ...
    REQUIRE_TRUE(app_tasks_telemetry_queue_count(&context) == 1u, "ESTOP response should queue telemetry");
    REQUIRE_TRUE(app_telemetry_task_step(&context) == APP_TASK_STATUS_OK, "telemetry task should emit ESTOP response");
    ...
    REQUIRE_TRUE(app_telemetry_task_step(&context) == APP_TASK_STATUS_OK, "telemetry task should emit faulted MOVE response");
    ...
    REQUIRE_TRUE(app_telemetry_task_step(&context) == APP_TASK_STATUS_OK, "telemetry task should emit CLEAR_FAULT response");
...
static int test_out_of_bounds_move_faults_system(void)
{
    ...
    REQUIRE_TRUE(app_tasks_telemetry_queue_count(&context) == 1u, "limit fault response should queue telemetry");
    REQUIRE_TRUE(app_telemetry_task_step(&context) == APP_TASK_STATUS_OK, "telemetry task should emit limit fault response");
...
static int test_telemetry_task_reports_snapshot(void)
{
    ...
    REQUIRE_TRUE(app_tasks_telemetry_queue_count(&context) == 0u, "telemetry task should drain queued responses");
    return 0;
}
